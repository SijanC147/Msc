...
from utils import token_filter, re_distribute, inspect_distribution
    ...
    def get_features_and_labels(self, mode, distribution=None):
        ...
        if self.features_labels_save_file_exists(mode):
            ...
            for index in range(len(dataset_dict["sentences"])):
                ...
                label = (
                    int(dataset_dict["labels"][index].strip())
                    if type(dataset_dict["labels"][index]) == str
                    else dataset_dict["labels"][index]
                )

                features["sentence"].append(sentence)
                features["target"].append(target)
                labels.append(label)
                left_context, right_context = self.get_left_and_right_contexts(
                    sentence=sentence,
                    target=target,
                ...
                if index % 60 == 0:
                    print(
                        "Processed {0}/{1} lines ({2:.2f}%) tot:{3:.3f}s avg:{4:.3f}s/line".format(
                            index + 1,
                            len(dataset_dict["sentences"]),
                            ((index + 1) / len(dataset_dict["sentences"]))
                            * 100,
                            total_time,
                            total_time / (index + 1),
                        )
        ...
        if distribution is not None:
            features, labels = re_distribute(features, labels, distribution)

        stats = inspect_distribution(features=features, labels=labels)
