...
class FeatureProvider:
    def __init__(self, datasets, embedding, oov=None, oov_buckets=0):
        ...
        self._init_token_data(oov, oov_buckets)
    ...
    def _init_vocab(self, embedding, oov, oov_buckets):
        ...
        train_vocab = list(map(str.lower, [*self._train_corpus]))
        if exists(self._vocab_file):
            ...
            vocab_tsv_file = vocab_file.format(ext=".tsv")
        ...
        temp_vocab_file = vocab_file.format(ext=".filt.txt")
        temp_vocab_path = join(self._gen_dir, temp_vocab_file)
        if not exists(temp_vocab_path):
            temp_vocab = list(set(train_vocab) & set(self._vocab))
            indices = [self._vocab.index(word) for word in temp_vocab]
            write_vocab_file(temp_vocab_path, temp_vocab, indices=indices)
        return temp_vocab_path

    def _init_token_data(self, oov, oov_buckets):
        to_tokenize = {}
        for mode in ["train", "test"]:
        ...
        if to_tokenize:
            include = self._vocab
            if oov or oov_buckets:
                train_vocab = list(map(str.lower, [*self._train_corpus]))
                test_vocab = list(map(str.lower, [*self._test_corpus]))
                oov_tokens = set(train_vocab + test_vocab)
                include = list(set(self._vocab) | oov_tokens)
